...
export function makeAtlasVerify(HOST, CTX) {
  ...
  const PINPASS_BUDGET_MS = 20000;
    ...
    /* ⚠⚠ (#R540) AN UNPLACED PLACE CARRIES WHY IT IS UNPLACED. 「正確に特定できませんでした」 was
    ...
    function _atlMappingNoteHtml(v, src, meta){ meta=meta||{}; let h='';
      ...
      /* (#R540) ONE LINE PER CAUSE. `unplacedBy` is absent only for a verdict built by an older
    ...
    function makePinReplyPlaces(D) {
      ...
      try{
        ...
        for(const it of struct.concat(textCands)){
          ...
          if(newPins.length>=14){ spots.push({name:it.name,verdict:'unplaced',reason:'budget',src:it.src}); continue; }   /* (#R540) the 14-pin cap is OUR limit, not a fact about this place */
          if((Date.now()-pin0)>=PINPASS_BUDGET_MS){ spots.push({name:it.name,verdict:'unplaced',reason:'budget',src:it.src}); continue; }   /* (#R540) …and so is the deadline */
          ...
          if(!g&&ledger){ try{ const k=ledger.resolve(it.name,{kind:it.kind,countryName:it.country}); if(k&&k.lng!=null) g={lng:k.lng,lat:k.lat,name:k.canonicalName||k.name}; }catch(_){} }   /* (#R489) a place THIS conversation already resolved is not sent to a geocoder again. ⚠ (#R540) THE HINT IS BUILT FROM WHAT THIS MAPPER HOLDS — it used to pass `countryCode: it.countryCode`, and the mapper above copies no country code at all, so the narrowing was undefined at every call and 「モスクワ」 could come back as whichever one was recorded last */
