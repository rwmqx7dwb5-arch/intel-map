...
export function makeLazyModules(HOST) {
  return (function () {
    ...
    const PUBLISHES = {
      ...
      atlasConsole: 'IntMapConsole', atlasQuery: 'IntMapQuery', atlasChart: 'IntMapAtlasChart', atlasAnswerView: 'IntMapAnswerView',   /* (#R540) the chart renderer rides on this line for the same reason the query engine does: a chart is drawn only by an answer that decided to draw one, and js/atlas-console.js's chunk has 4,901 bytes of budget left (tests/perf-baseline.json), which is less than the renderer. (#R495) the cross-dataset query engine and, through its static import, the coastline it measures 「海から200km」 with — behind its OWN door rather than inside the Atlas chunk, so a session that opens Atlas to ask about one place never pays for a 249 kB coastline and a 2 MB index it does not query. ⚠ ON THIS LINE because tests/r168 #8 budgets this file as part of the shell, and five separate lines put it over. */
