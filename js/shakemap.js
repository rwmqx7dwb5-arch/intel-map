...
window.IntMapModules.shakeMap = function (HOST) {
  ...
  async function open(eventId, metricWanted) {
    ...
    /* ⚠ (#R548) THE LEGEND IS PART OF OPENING, NOT A SEPARATE COURTESY. It was
  ...
  function mountLegend() {
    ...
    if (!el) return null;
    /* ⚠ (#R548) …AND THE HEADING HAS TO BE WRITTEN, NOT JUST PASSED. The name
    ...
    box.querySelectorAll('[data-shk-m]').forEach(b => { b.onclick = () => { open(cur.id, b.getAttribute('data-shk-m')).catch(() => { });   /* (#R548) `open` mounts the legend itself now */ }; });
  ...
  /* kept as the map click handler's door, and as the name the dispatch calls; opening
     IS drawing the legend since #R548, so this is now `open` under its other name. */
