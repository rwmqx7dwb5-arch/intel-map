...
,
  ...
  sourceUse: {
    ...
    "GeoNames": "The bundled world gazetteer data/gazetteer-world.json.gz — 147,924 populated places across 242 countries (coordinates, population and country from the cities1000 export; names in 18 languages from alternateNamesV2, which unlike the inline column carries an ISO language code per name), gzipped to 3.9 MB and un-gzipped in the browser. Built by scripts/build-gazetteer.mjs. It is what lets the non-AI news locator and the search box resolve a place ten times further down the long tail than the curated table alone. CC BY 4.0. A second bundled export from the same source answers a different question: data/histcities-homonyms.json.gz is built from cities500 (not cities1000) and keeps, for each spelling the historical city record joins on, every settlement on Earth that answers to that spelling — no dedup by population, so whatever is genuinely ambiguous stays ambiguous. It is the build-time evidence for which city a historical city label names, read by npm run check:histcities and never served to the browser. Same licence, CC BY 4.0.",
    ...
    "Yahoo Finance": "Stock-index quotes for the bottom ticker, and live share prices used to compute live market caps in the Companies tab (US-listed names). The Companies tab asks IntMap's own quotes-relay Edge Function first and drops to the shared public CORS relays only when that cannot be reached; upstream is Yahoo's keyless spark/chart endpoints either way, and a ticker symbol is the whole of what is sent — no coordinates, no account, nothing about you",
    "Company logos — Wikidata (P154) via Wikimedia Commons, Google favicons as fallback": "The logo beside each company in the Companies tab. WHICH image is a company's logo is Wikidata's answer — property P154, resolved at BUILD time and written into data/companies/, so 435 of the 533 companies carry one and nothing is asked of Wikidata while you read: it never learns which company you are looking at. The image itself is served by Wikimedia Commons (Special:FilePath, which redirects to upload.wikimedia.org). For a company Commons has no logo for, the fallback is Google's favicon service, and there the company's DOMAIN NAME — nothing else — is sent to Google. With neither, a monogram of the company's initials is drawn, which makes no external request at all. Each logo is a trade mark of the company it identifies and is used here only to identify it; the licence is per FILE on Commons (most wordmarks fall below the threshold of originality and are public domain, some are under a CC licence). ⚠ This replaced the Clearbit Logo API, shut down on 2025-12-08: logo.clearbit.com no longer resolves at all, and one opening of the Companies tab produced 189 name-resolution failures.",
...
"Smithsonian GVP": "Bundled with the app (data/volcanoes_gvp.json + data/volcano-detail.json.gz, built by scripts/build-volcanoes.mjs from the GVP WFS): the whole Holocene catalog with its join key, and behind it every eruption GVP records for the volcanoes in that catalog, with its VEI and dates, the volcano type, landform, tectonic setting and dominant rock type, the geological summary, the photograph, and the population living within 5, 10, 30 and 100 km. Everything the volcano card says about the past is read from this file, offline. The catalog is the Holocene list plus the volcanoes an observatory publishes a current alert level for: Yellowstone, Long Valley, Coso Volcanic Field and Isanotski Peaks are older than the Holocene and come from the GVP Pleistocene catalog, so that a volcano the USGS reports on every day can be drawn.",
    ...
    "Religion and language composition by country — CIA World Factbook (US Government work, public domain)": "Bundled with the app: the Factbook’s own «Religions» and «Languages» fields, parsed into a share per group per country — 202 countries for religion and 204 for language, each with the year the Factbook states. It is what lets the religion layer separate Catholic, Protestant and Orthodox. Where the Factbook does NOT separate the denominations — the United Kingdom’s entry reads «Christian (includes Anglican, Roman Catholic, Presbyterian, Methodist)» — neither does the map.",
    "Language identity, genealogical classification and endangerment status — Glottolog (CC BY 4.0)": "Bundled with the app: Glottolog’s languoid catalogue — every language and language family, each with its Glottocode, its parent, its ISO 639-3 code and its endangerment status. This is what gives the language layer an identity to be right about. Under the ISO 639-1 tags it replaced, «Creole» in Mauritius and «Creole» in Haiti were the same code, Kirundi was recorded as Kinyarwanda, and six Sinitic languages shared one tag; and because the old table simply dropped whatever it did not recognise, Burkina Faso was drawn as Fula 7.8% with Mossi 52.9% missing. Every name the Factbook prints is now resolved to a Glottocode by a stated rule or by a written decision in data/language-aliases.json, and a name that is neither fails the build.",
