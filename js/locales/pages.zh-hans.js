...
,
  ...
  sourceUse: {
    ...
    "GeoNames": "随程序打包的世界地名词典 data/gazetteer-world.json.gz——242 个国家、147,924 个聚落（坐标、人口与国别取自 cities1000 导出档；18 种语言的名称取自 alternateNamesV2，与内嵌字段不同，它为每个名称附上 ISO 语言代码），以 gzip 压缩为 3.9 MB，于浏览器端解压。由 scripts/build-gazetteer.mjs 建立。正因如此，非 AI 的新闻定位器与搜索框才能比仅用精选表格多解析十倍长尾的地名。CC BY 4.0。同一个来源还随程序打包了第二个导出档，用来回答另一个问题：data/histcities-homonyms.json.gz 由 cities500（不是 cities1000）建立，对历史城市名数据用来比对的 1,034 个拼法，保留世界上所有叫这个名字的聚落——不依人口剔除同名者，因此真正有歧义的仍然保持歧义。它是在建置时用来验证历史城市名标签究竟指哪一座城市的证据，由 npm run check:histcities 读取，不会送到浏览器。授权相同，CC BY 4.0。",
