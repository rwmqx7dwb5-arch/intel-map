...
window.IntMapPageI18N.define('es', {
  ...
  sourceUse: {
    ...
    'GeoNames': 'El nomenclátor mundial incluido data/gazetteer-world.json.gz: 147.924 lugares habitados en 242 países (coordenadas, población y país de la exportación cities1000; nombres en 18 idiomas de alternateNamesV2, que a diferencia de la columna en línea lleva un código de idioma ISO por nombre), comprimido a 3,9 MB y descomprimido en el navegador. Generado por scripts/build-gazetteer.mjs. Es lo que permite que la localización de noticias sin IA y el cuadro de búsqueda resuelvan lugares diez veces más lejos en la cola larga que la tabla curada por sí sola. CC BY 4.0. De la misma fuente se incluye una segunda exportación que responde a otra pregunta: data/histcities-homonyms.json.gz se genera a partir de cities500 (no de cities1000) y conserva, para cada una de las grafías con las que enlaza el registro de nombres históricos de ciudades, todos los asentamientos del mundo que responden a esa grafía, sin descartar homónimos por población, de modo que lo realmente ambiguo sigue siendo ambiguo. Es la prueba en tiempo de compilación de a qué ciudad se refiere una etiqueta de nombre histórico; la lee npm run check:histcities y nunca se envía al navegador. Misma licencia, CC BY 4.0.',
    ...
    'Yahoo Finance': 'Cotizaciones de índices bursátiles para el teletipo inferior y precios de acciones en vivo para calcular la capitalización de mercado en la pestaña Empresas (valores cotizados en EE. UU.). La pestaña consulta primero la Edge Function propia de IntMap, quotes-relay, y solo recurre a los relés CORS públicos compartidos cuando esta no está disponible; el origen sigue siendo en ambos casos los extremos sin clave spark/chart de Yahoo, y lo único que se envía es el símbolo del valor: ni coordenadas, ni cuenta, ni nada sobre usted.',
    'Company logos — Wikidata (P154) via Wikimedia Commons, Google favicons as fallback': 'El logotipo que acompaña a cada empresa en la pestaña Empresas. QUÉ imagen es el logotipo de una empresa lo responde Wikidata con la propiedad P154, resuelta en tiempo de COMPILACIÓN y escrita en data/companies/: 435 de las 533 empresas tienen uno y, mientras usted lee, no se pregunta nada a Wikidata, que nunca sabe qué empresa está mirando. La imagen la sirve Wikimedia Commons (Special:FilePath, que redirige a upload.wikimedia.org). Para una empresa sin logotipo en Commons, el respaldo es el servicio de favicons de Google, y allí se envía únicamente el NOMBRE DE DOMINIO de la empresa. Sin ninguno de los dos se dibuja un monograma con sus iniciales, que no genera ninguna petición externa. Las marcas pertenecen a las empresas que identifican y aquí solo se usan para identificarlas; la licencia consta en cada ARCHIVO de Commons (la mayoría de los logotipos tipográficos no alcanzan el umbral de originalidad y son de dominio público; algunos están bajo licencia CC). ⚠ Esto sustituye a la Clearbit Logo API, cerrada el 2025-12-08: logo.clearbit.com ya no se resuelve, y una sola apertura de la pestaña producía 189 fallos de resolución de nombre.',
...
'Smithsonian GVP': 'Incluido con la aplicación (data/volcanoes_gvp.json y data/volcano-detail.json.gz, generados por scripts/build-volcanoes.mjs a partir del WFS del GVP): todo el catálogo del Holoceno con su clave de unión y, detrás, todas las erupciones que el GVP registra para los volcanes de ese catálogo, con su VEI y sus fechas, el tipo de volcán, la forma del relieve, el contexto tectónico y la roca dominante, el resumen geológico, la fotografía y la población que vive en un radio de 5, 10, 30 y 100 km. Todo lo que la ficha del volcán dice del pasado se lee de este archivo, sin conexión. El catálogo es la lista del Holoceno más los volcanes para los que un observatorio publica un nivel de alerta actual: Yellowstone, Long Valley, el campo volcánico de Coso y los picos Isanotski son más antiguos que el Holoceno y proceden del catálogo del Pleistoceno del GVP, para que también pueda dibujarse un volcán del que el USGS informa cada día.',
    ...
    "Religion and language composition by country — CIA World Factbook (US Government work, public domain)": "Incluido: los campos «Religions» y «Languages» del propio Factbook, convertidos en cuota por grupo y país (202 y 204 países). Donde el Factbook no separa las confesiones, el mapa tampoco.",
    "Language identity, genealogical classification and endangerment status — Glottolog (CC BY 4.0)": "Incluido: el catálogo de lenguoides de Glottolog — cada lengua y cada familia lingüística, con su Glottocode, su nodo padre, su código ISO 639-3 y su estado de vitalidad. Es lo que da a la capa de idiomas una identidad sobre la que acertar. Con las etiquetas ISO 639-1 que sustituye, el «criollo» de Mauricio y el de Haití compartían código, el kirundi se registraba como kinyarwanda y seis lenguas siníticas compartían una etiqueta; y como la tabla anterior descartaba en silencio lo que no reconocía, Burkina Faso aparecía como fula 7,8 % con el mossi 52,9 % ausente. Todo nombre que imprime el Factbook se resuelve ahora a un Glottocode por una regla declarada o por una decisión escrita en data/language-aliases.json, y un nombre que no tenga ninguna de las dos hace fallar la compilación.",
