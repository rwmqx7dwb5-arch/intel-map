...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    async function dispatch(a,dctx){ if(!a||!a.type) return R(true,''); switch(a.type){
        ...
        case 'compose': case 'mapCompose': case 'composeMap': case 'explainOnMap': return await COMPOSE.run(a,dctx);   /* (#R551) the execution context — which TURN this is — reaches the module, so two composes in one turn are two revisions of ONE map rather than two maps */   case 'shakemap': case 'shakeMap': case 'groundShaking': case 'intensityMap': { await window.IntMapLazy.need('shakeMap'); const _sk=await window.IntMapShakeMap.run(a); return R(_sk.ok,_sk.html,_sk.meta); }   /* (#R546) map.shakemap — js/shakemap.js owns the body BECAUSE this file has no line left */   /* (#R511) map.compose — js/atlas-map-compose.js. ⚠ THE LINE CAME FROM A BLANK ONE ABOVE THE TIME-AXIS BLOCK: this file is at its ceiling (tests/r318 ⓑ) */
    ...
    const OVL_OF={ highlight:'highlight', mapMetric:'choropleth', choropleth:'choropleth', poi:'poi', elevationBelow:'elevation', belowSeaLevel:'elevation', elevationHighlight:'elevation', elevationScan:'elevation',
      ...
      isochrone:'isochrone', reach:'isochrone', reachability:'isochrone', reachable:'isochrone', catchment:'isochrone', compose:'compose', mapCompose:'compose', composeMap:'compose', explainOnMap:'compose', shakemap:'shakemap', shakeMap:'shakemap', groundShaking:'shakemap', intensityMap:'shakemap' };   /* (#R511) */
