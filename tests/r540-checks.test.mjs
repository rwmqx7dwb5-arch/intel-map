/* ============================================================================
 *  R540 — THE HINT A CALLER CAN ACTUALLY SUPPLY, AND THE REASON A PLACE IS UNPLACED
 * ----------------------------------------------------------------------------
...
test('R540 ①a: the ledger declines a place it holds for a DIFFERENT country', () => {
  const led = ledgerOf();
...
test('R540 ①b0: …and the pin pass then falls through to the geocoder with the right country', async () => {
  const led = ledgerOf();
  led.record({ kind: 'city', name: 'Springfield', canonicalName: 'Springfield', countryName: 'United States', lng: -89.64, lat: 39.80, source: 'answer', provenance: 'geocoded_point' });
  const r = await run({
    ledger: led,
...
test('R540 ①b: an entity that does not know its country is not excluded by a country hint', () => {
  const led = ledgerOf();
...
test('R540 ①c: the kind hint still narrows, and the pin pass supplies both keys it holds', async () => {
  const led = ledgerOf();
...
test('R540 ②a: a name nothing could resolve is the only one called “couldn’t locate precisely”', async () => {
  const r = await run({
    ledger: ledgerOf(),
    geocode: false,
    nominatim: () => [],
...
test('R540 ②b: the 14-pin cap is reported as OUR limit, not as a fact about the place', async () => {
...
test('R540 ②c: a lookup that never answered is reported as the lookup, not as the place', async () => {
  const r = await run({
    ledger: ledgerOf(),
    geocode: 'throw',
    nominatim: 'dead',
...
test('R540 ②d: a verdict built the old way still prints the line it always printed', () => {
...
test('R540 ②e: every unplaced name still appears in `unplaced`, whatever its reason', () => {
