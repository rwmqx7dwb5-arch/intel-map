...
- **本体は `index.html`（988行・96 KB）＋ `css/`（3本）＋ `js/`（256本・11.3 MB）＋ `src/`（10本）。**
...
| 部品 | ファイル | 何の正本か |
|---|---|---|
| Capability Registry | `js/atlas-capabilities.js` | **130 能力**。ID・別名（**440 綴り**＝ID＋別名の重複を除いた実測。**照合は camelCase を語に割ってから**——割らないと `myLocation` は「my location」で引けず、実測 143 綴り中 60 がどの言語からも届かなかった）・分類・副作用（`writes`＝競合キー）・生成物・危険度・確認要否・**必要な対象**・遅延モジュール・観測器・検証器 |
| 能力の説明文 | `js/atlas-catalog-text.js` | 46 ブロック。**各ブロックがどの能力を説明しているか**を持つ。`find_capability` が要求されたときだけ返す |
| 引数の schema | `js/atlas-schemas.js` | **130 能力ぶんの引数定義**。型・列挙・範囲と、`required` / `anyOf`（「地点 か 緯度経度」）|
...
| 道具の面 | `js/atlas-toolsurface.js` | そのターンに渡す**中核 8 ツール**（`my_location`、画面そのものを見る `look_at_map`、地図説明を 1 回で描く `compose_map` を含む）＋`find_capability`（レジストリの全 130 を検索・返るのは撤去済み 1 を除く **129** から・**打ち切り無し**）／`run_capability`（ID 指定で起動）＝計 10 本。`ask_user` は `endsTurn`＝**ターンを終える道具**で、旗は**結果にも**載る（`run_capability` が `dialog.ask` を ID で呼ぶ経路では、呼びの名前は `run_capability` だから）。同じ場所で結果に `changedMap` を刻む——**その能力が `map` を生成し、観測器が completed と言ったとき**だけ。監査に削られた回答は `status:'degraded'` と削除件数で返す |
| 地図説明の合成 | `js/atlas-map-compose.js` | **`map.compose`（tool 名 `compose_map`）は「地図で説明する」という 1 つの行為を 1 回の呼び出しにしたもの。** 地点（番号順・役割つき）・地点間の関係（大円の弧。flow / route は矢印、influence / border は破線）・塗り分け（highlight 経路へ委譲）・全体を収めるカメラ・同じ番号の凡例。地名は**台帳 → ジオコーダ**の順にコードが解決し（国名は既に含まれていなければ 1 回だけ付け、国名付きで見つからなければ裸の名前で再試行する——海峡は国の中に無い）、解決したものは**役割ごと**台帳へ戻す。**地名の解決は 3 段**——①この会話が既に解決した台帳、②ジオコーダ、③**ジオコーダが名前を持たなかったものは、実ウェブ検索に裏づけられた 1 回の照会**（`geoVerifyMany`。**外した名前すべてを 1 つの問いに載せ、読者が既に支払ったターンの中で**訊く——名前ごとに訊くと 1 日の利用枠を 1 件につき 1 つ消費する）。③で置いた点は `provenance:"web_verified"` として記録され、**gazetteer の地物と区別できるまま**残る。⚠ **どの段もモデルの書いた座標は受け取らない。** ②の**ジオコーダは候補を 8 件取り、自分の名前（`namedetails` の各言語の別名を含む）が問い合わせと一致した候補だけを残す**——一致が無ければ**何も返さない**。Nominatim の全文検索は一致しない語を落として残りを順位づけるため、OSM にその名前で存在しない地点は**他人を 200 OK で返す**（「宇部港」が浜松市の郵便ポスト、「岩国・大竹地区」が築地の病院）。解決できなかった地名は **`unplaced` に名前で**残り、**問い合わせに使った綴りが `tried` として付く**（別名で呼び直すかどうかは Atlas が決める）。Atlas にも読者にも見える——座標は発明しないし、他人を代役にも立てない。**関係の線は大円の弧であって実測経路ではなく、どんな関係なら線を引いてよいかは Atlas の判断**（同じ産業・同じ地方に属することは接続ではない、と catalogue が明示する）。描画元は `atl-compose-src` 1 本で、`paintNow()` がそれを数える。`linkProse()` が回答文の**最初の言及**に番号バッジを付け（テキストノードだけ・リンクやコードの中は触らない）、hover で地図の印と双方向に光る |
...
`find_capability(query)` が**レジストリの全 130 を検索**し（返るのは撤去済み 1 を除く **129** から）、**得点したものを全部** schema 付きで返し（**打ち切り無し**——説明文は 46 ブロック共有なので、能力ごとに引くと同じブロックが繰り返される。**まとめて 1 回引いて重複を落とす**：実測 67,600 → 24,519 B・1 文字も切らずに）、`run_capability(id, args)`
...
- **下限は 1850 年**（`IntMapTime.min`）。⚠ **この数を書き写さない**——スライダーの `min`・入力の
  ...
  歴史国境は CShapes 2.0 が 1886–2019、それ以前はスナップショット（historical-basemaps の
  **1815 と 1880 の2枚だけ**——上流にその間は無い。**時計から実際に届くのは 1880 の1枚だけ**で、1815 が
  選ばれるのは 1847.5 年より前、つまり下限の外だけ。したがって **1850–1885 はどの年も 1880 年の国境**で
  描かれ、出典ページがそう書いている）／GDP・人口はマディソン・プロジェクトで 1850 年から／
...
- **都市名ラベルも時計に従う**（`js/hist-cities.js` の `window.IntMapHistCities`・記録は
  ...
  **レニングラード**。⚠ **層を足していない**——`ofm-city` の `text-field` を `match` で包み、**既定は
  ...
  ⚠ **適用先は `ofm-city`（`class in [city, town]`）だけ**。記録の免除理由はこの絞り込みを前提に書いてある。
- **綴りが2都市を指してはならない**——ラベルの書き換えはタイル自身の名前との一致で起きるので、
  他の都市が同じ綴りを持てばそちらも改名されてしまう。`scripts/build-hist-cities.mjs`（`npm run
  check:histcities`）が全キーを `data/gazetteer-world.json.gz` と照合し、**40 km 以内に本人が居ること**と
  **2万人以上の別の集落が同じ綴りを持たないこと**を要求する。例外は行に `!` を付けて**理由を書いた場合だけ**。
  実測でこれが落とした行がある（アルメニアのアルマヴィル＝クラスノダール地方のアルマヴィルと同名、
  ジンバブエのカドマ＝大阪府門真市と同綴り、テリヨキ＝クラスノヤルスク地方のゼレノゴルスクと同名）。
...
- **両大戦の日ごとの勢力**（`js/war-fronts.js`・レイヤー行は **`dl-ww1` と `dl-ww2` の2本**・
  ...
  - **収録範囲**: 第一次大戦 **9 戦線 / 85 本の日付入り戦線 / 195 作戦 / 124 領域**、第二次大戦
    **12 戦線 / 109 本 / 313 作戦 / 156 領域**。地名辞書は **642 件**。
    作戦のうち **死傷・捕虜の数値を持つのが 323 件・投入兵力が 203 件**。
  ...
  - ⚠ **太平洋には戦線を引かない**——線が存在しなかったから。島嶼戦は「どの場所がいつ手を変えたか」
    ...
    集合そのもの**で、東経100度以東の作戦 **80 件**が 1939–45 の各年に分布する。
