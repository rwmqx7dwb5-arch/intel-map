...
/* ⚠ (#R540) THE CORE CEILING FELL, 35 -> 31, AND THE FALL IS NOT A SAVING THIS ROUND MADE. The gate
   ...
   this reason) — a ceiling left at the height of the round that happened to be most expensive stops
   measuring anything. ⚠ AND THE NEW SPEC IS IN THE GATE ONLY BECAUSE OF ITS NAME: its entry is 5,
   well over CORE_MAX_S = 1, so it stands here as the current round's spec and nothing else. It is
   named tests/r540.spec.js rather than tests/r540-correlate.spec.js precisely so that it does —
   `currentRoundSpec()` matches /^r(\d+)$/, and a regression test that only runs at night does not
   guard the push it was written for. */
const BUDGET_S = 31;                    /* core: 0.5 min — measured 31 s over 7 files (#R540) */
...
/* ⚠⚠ (#R530) THE TOTAL CEILING MOVED, BY THE MEASURED AMOUNT — 4,639 -> 4,648 (+9 s). Saying it here
   ...
   note above). */
/* ⚠⚠ (#R540) THE TOTAL CEILING MOVED, BY THE MEASURED AMOUNT — 4,648 -> 4,653 (+5). Saying it here
   ...
   is a promise with no rejection arm.
     ...
     · CALIBRATED the way #R451, #R474 and #R508 did rather than copied. Two committed entries were
       ...
       for a file that costs «less than half» of r494's, i.e. that entry already carries a
       conservative +1, and dividing by it counts the same padding twice. A third, independent
       check settles it — tests/r530.spec.js (entry 9) measured 7.149 s here against the 6.6/7.0/
       8.1/7.5 s #R530 recorded on a quiet machine, so this machine is not slower than the one that
       wrote these entries and the gap between local and table seconds is ROUNDING, not hardware.
...
const TOTAL_BUDGET_S = 4653;            /* 77.5 min — 4,639 (#R510) + 9 (#R530) + 5 (#R540: tests/r540.spec.js) */
